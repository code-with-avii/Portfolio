...
import { fetchMessages, toggleMessageRead, deleteMessage } from '../store/slices/messageSlice.js';
import { LogOut, Inbox, Trash2, MailOpen, Mail, ArrowLeft, ShieldCheck, Clock, CheckCircle } from 'lucide-react';
...
export default function AdminDashboard() {
 ...
 <h1 className="text-2xl font-bold font-heading text-white">Inbound Messages</h1>
 <p className="text-xs text-zinc-500 font-semibold mt-1">Review contact inquiries left by recruiters.</p>
 </div>
 ...
 <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Total Mail</span>
 </div>
 <div className="text-xl font-bold font-heading text-white">{messages.length}</div>
 </div>
 ...
 <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Unread Mail</span>
 </div>
 <div className="text-xl font-bold font-heading text-white">{unreadCount}</div>
 </div>
 </div>
 ...
 <span className="font-heading font-bold text-xs text-white truncate max-w-[150px]">
 {msg.name}
 </span>
 ...
 <h4 className={`text-xs mt-2 truncate ${msg.isRead ? 'text-zinc-400 font-semibold' : 'text-cyan-400 font-bold'}`}>
 {msg.subject}
 </h4>
 ...
 {selectedMessage ? (
 <div className="glass-panel p-6 sm:p-8 rounded-2xl border border-white/5 flex flex-col justify-between min-h-[400px]">
 
 <div>
 {/* Sender title */}
 <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/5 pb-4 mb-6">
 <div>
 <h2 className="text-lg font-heading font-bold text-white leading-snug">{selectedMessage.name}</h2>
 <span className="text-xs text-cyan-400 font-code font-semibold">{selectedMessage.email}</span>
 </div>
 ...
 selectedMessage.isRead ? 'border-zinc-800 bg-zinc-900/50' : 'border-cyan-500/20 bg-cyan-500/5 text-cyan-400'
 }`}
 title={selectedMessage.isRead ? 'Mark as unread' : 'Mark as read'}
 >
 {selectedMessage.isRead ? <MailOpen size={15} /> : <Mail size={15} />}
 </button>
 ...
 <div>
 <div className="text-[10px] uppercase font-bold text-zinc-500 mb-1">Subject</div>
 <h3 className="text-sm font-bold text-white leading-normal">{selectedMessage.subject}</h3>
 </div>
 
 <div>
 <div className="text-[10px] uppercase font-bold text-zinc-500 mb-1.5">Message Content</div>
 ...
 <>
 <CheckCircle size={10} className="text-emerald-400" /> Reviewed
 </>
 ) : (
 'Inbound Queue'
 )}
 ...
 <div className="glass-panel p-12 rounded-2xl border border-white/5 text-center text-zinc-500 text-sm flex items-center justify-center min-h-[400px] font-body select-none">
