...
const AnimatedCounter = ({ value, label, duration = 2 }) => {
 ...
 <div className="text-xs text-mutedText font-semibold mt-2 uppercase tracking-wider">{label}</div>
 </div>
 );
};
...
const categoryColors= {
 ...
 'GitHub Milestones': 'border-emerald-500/20 text-emerald-400 bg-emerald-500/5',
...
export default function Achievements() {
 ...
 >
 A validation checklist of certifications, platform ranks, and hackathon milestones.
 </motion.p>
 </div>

 {/* Counters Grid */}
 <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto mb-16">
 <AnimatedCounter value={15} label="Projects Built" />
 ...
 categoryColors[ach.category] || 'border-zinc-800 text-zinc-400 bg-zinc-800/10'
 }`}>
 {categoryIcons[ach.category]}
 {ach.category}
 </span>
 <span className="text-[10px] text-mutedText font-code font-bold">{ach.date}</span>
 </div>

 {/* Title & Badge Value */}
 <h3 className="text-base sm:text-lg font-heading font-bold text-white leading-snug">{ach.title}</h3>
 <div className="text-cyan-400 text-xs font-semibold font-code mt-1">{ach.value}</div>

 {/* Description */}
 <p className="text-xs sm:text-sm text-zinc-400 font-body leading-relaxed mt-4">{ach.description}</p>
 </div>
