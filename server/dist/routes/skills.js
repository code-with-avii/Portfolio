...
router.post('/', authenticateToken, async (req, res) => {
    ...
    try {
        if (isDbConnected) {
            const newSkill = await Skill.create({ name, category, icon, currentlyLearning });
...
router.put('/:id', authenticateToken, async (req, res) => {
    ...
    try {
        if (isDbConnected) {
            const updatedSkill = await Skill.findByIdAndUpdate(req.params.id, { name, category, icon, currentlyLearning }, { new: true });
            if (!updatedSkill)
                return res.status(404).json({ error: 'Skill not found.' });
            return res.json(updatedSkill);
        }
        else {
            const skillIdx = memoryStore.skills.findIndex(s => s._id === req.params.id);
            if (skillIdx === -1)
                return res.status(404).json({ error: 'Skill not found.' });
            memoryStore.skills[skillIdx] = {
                ...memoryStore.skills[skillIdx],
                name: name !== undefined ? name : memoryStore.skills[skillIdx].name,
                category: category !== undefined ? category : memoryStore.skills[skillIdx].category,
                icon: icon !== undefined ? icon : memoryStore.skills[skillIdx].icon,
                currentlyLearning: currentlyLearning !== undefined ? !!currentlyLearning : memoryStore.skills[skillIdx].currentlyLearning,
                updatedAt: new Date()
            };
