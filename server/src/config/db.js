...
import { initialSkills, initialProjects, initialExperiences, initialAchievements } from '../data/initialData.js';
...
memoryStore.skills.forEach((s, idx) => { s._id = `mem-skill-${idx}`; });
memoryStore.projects.forEach((p, idx) => { p._id = `mem-project-${idx}`; });
memoryStore.experiences.forEach((e, idx) => { e._id = `mem-exp-${idx}`; });
memoryStore.achievements.forEach((a, idx) => { a._id = `mem-ach-${idx}`; });
