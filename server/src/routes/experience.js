...
router.post('/', authenticateToken, async (req, res) => {
 const { role, company, duration, type, technologies, achievements } = req.body;
 if (!role || !company || !duration || !type) {
 return res.status(400).json({ error: 'Role, company, duration, and type are required.' });
 }

 try {
 if (isDbConnected) {
 const newExp = await Experience.create({ role, company, duration, type, technologies, achievements });
...
router.put('/:id', authenticateToken, async (req, res) => {
 ...
 try {
 if (isDbConnected) {
 const updatedExp = await Experience.findByIdAndUpdate(req.params.id, fields, { new: true });
 if (!updatedExp) return res.status(404).json({ error: 'Experience not found.' });
 return res.json(updatedExp);
 } else {
 const expIdx = memoryStore.experiences.findIndex(e => e._id === req.params.id);
 if (expIdx === -1) return res.status(404).json({ error: 'Experience not found.' });
...
router.delete('/:id', authenticateToken, async (req, res) => {
 ...
 if (!deletedExp) return res.status(404).json({ error: 'Experience not found.' });
